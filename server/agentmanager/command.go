	...
	if err == nil {
		for _, result := range cmdresults {
			if result.RetCode == 1 && strings.Contains(result.Stdout, "is not installed") && result.Stderr == "" {
				logger.Error("%s not installed happened when running getpkgversion: %s, %s, %s; ", pkgname, result.MachineUUID, result.MachineIP, result.Stderr)
				continue
			} else if result.RetCode == 127 && result.Stdout == "" && strings.Contains(result.Stderr, "command not found") {
				logger.Error("rpm not installed when running getpkgversion: %s, %s, %s", result.MachineUUID, result.MachineIP, result.Stderr)
				continue
				...
				if err != nil && len(v) != 0 {
					logger.Error("failed to read RPM package version when running getpkgversion: %s, %s, %s", result.MachineUUID, result.MachineIP, result.Stderr)
					continue
				}
				...
				logger.Error("failed to run command: rpm -qi %s in %s, %s, %s when running getpkgversion", pkgname, result.MachineUUID, result.MachineIP, result.Stderr)
				continue
			}
		}
		return machines, nil
